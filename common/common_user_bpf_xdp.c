...
int xdp_link_detach(int ifindex, __u32 xdp_flags, __u32 expected_prog_id, bool verbose)
{
	__u32 curr_prog_id;
	int err;
...
struct bpf_object *load_bpf_and_xdp_attach(__u32 xdp_flags, int ifindex, bool reuse_maps, const char* filename, char *progsec, const char* pin_dir)
{
	...
	printf("sec: %s filename: %s, reuse: %d\n", progsec, filename, reuse_maps);
	...
	if (reuse_maps)
		bpf_obj = load_bpf_object_file_reuse_maps(filename,
							  offload_ifindex,
							  pin_dir);
	else
	...
	/* At this point: All XDP/BPF programs from the filename have been
	...
	strncpy(progsec, bpf_program__title(bpf_prog, false), 32);


	printf("sec: %s bfprog: %s\n", progsec, bpf_program__title(bpf_prog, false)); 
